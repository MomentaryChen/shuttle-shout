...
export function TeamCallingSystem() {
  ...
  useEffect(() => {
    if (urlTeamId && urlTeamId === teamId && teamId !== "" && !isNaN(Number(teamId)) && !team && !loading && isAuthenticated) {
      console.log("自動加載團隊並準備連接 WebSocket, teamId:", teamId)
      loadTeam()
    }
  ...
  const connectWebSocket = (teamData?: TeamDto) => {
    ...
    if (!currentTeam) {
      console.error("無法連接 WebSocket: 團隊信息未加載")
    ...
    try {
      ...
      console.log("正在連接 WebSocket:", fullWsUrl)
        ...
        try {
          ...
          if (data.type === "CONNECTED") {
            ...
            if (Array.isArray(assignments) && assignments.length > 0) {
              ...
              toast.success(`已為場地分配 ${newPlayers.length} 位人員${wasEmpty ? '，請確認後開始比賽' : ''}`)
            } else {
  ...
  const handleQueueUpdate = (queue: any[]) => {
    ...
    // 注意：後端只發送 userId, userName, userRealName, userEmail
    // 我們需要從 teamMembers 中查找完整信息，或使用部分信息
    const queueMembers: UserTeamDto[] = queue.map((item: any) => {
  ...
  // 處理場地更新
  const handleCourtUpdate = (courtData: Court) => {
    setCourts((prev) =>
      prev.map((court) => (court.id === courtData.id ? courtData : court))
    )
  }
  ...
  const assignPlayerToCourt = (member: UserTeamDto, courtId: number) => {
    // 檢查 WebSocket 連接狀態
    if (!wsConnected) {
      toast.warning("請先連接 WebSocket 以使用此功能")
      return
    }
  ...
  const confirmStartMatch = (courtId: number) => {
    // 檢查 WebSocket 連接狀態
    if (!wsConnected) {
      toast.warning("請先連接 WebSocket 以開始比賽")
      return
    }
    ...
    // 發送 WebSocket 消息，只傳遞 courtId 和 teamId，由後端選擇成員並開始比賽
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      const message = {
        type: "AUTO_ASSIGN",
        courtId,
      ...
      setCourts((prev) =>
        prev.map((c) => 
          c.id === courtId 
            ? { ...c, isPending: false, isActive: true }
            : c
        )
      )
      ...
      toast.success(`場地 ${court.name} 比賽已開始！`)
    } else {
      toast.error("WebSocket 未連接")
    }
  }
  ...
  const finishMatch = (courtId: number) => {
    // 檢查 WebSocket 連接狀態
    if (!wsConnected) {
      toast.warning("請先連接 WebSocket 以結束比賽")
      return
    }
            ...
            <div className="flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold text-blue-900 dark:text-blue-100 mb-2">
                  {wsConnecting ? "正在連接 WebSocket..." : "等待 WebSocket 連接"}
                </h3>
                <p className="text-sm text-muted-foreground">
                  {wsConnecting 
                    ? "系統正在建立實時連接，請稍候..." 
                    : "請點擊下方按鈕連接 WebSocket 以使用叫號系統"}
                </p>
              </div>
              {!wsConnecting && (
                <Button 
              ...
              return (
                ...
                >
                  ...
                  <CardContent className="space-y-4">
                    {/* 羽毛球场样式 */}
                    <div className="relative w-full aspect-[3/2] bg-gradient-to-b from-green-50 to-green-100 dark:from-green-950/30 dark:to-green-900/20 rounded-lg border-2 border-green-300 dark:border-green-800 overflow-hidden shadow-inner">
                      {/* 場地背景網格 */}
                      ...
                      {/* 位置1 - 左上（左边队伍） */}
                      <div className="absolute top-2 left-2 right-1/2 bottom-1/2 flex items-center justify-center p-1.5">
                        <div
                          className={`w-full h-full rounded-lg border-2 flex flex-col items-center justify-center p-1.5 transition-all duration-200 ${
                            player1
                              ? court.isPending
                          ...
                          onClick={player1 ? () => court.isPending ? removePlayerFromPendingCourt(player1.userId, court.id) : removePlayerFromCourt(player1.userId, court.id) : undefined}
                        >
                          {player1 ? (
                            <>
                              <Avatar className={`h-7 w-7 mb-0.5 border-2 ${teamLeftColor.avatarBorder}`}>
                                <AvatarFallback className={`${teamLeftColor.avatarBg} text-white text-[10px]`}>
                                  {(player1.userRealName || player1.userName).charAt(0).toUpperCase()}
                                </AvatarFallback>
                              </Avatar>
                              <p className="text-[10px] leading-tight font-semibold text-center break-words w-full px-0.5">
                      ...
                      {/* 位置2 - 右上（右边队伍） */}
                      <div className="absolute top-2 right-2 left-1/2 bottom-1/2 flex items-center justify-center p-1.5">
                        <div
                          className={`w-full h-full rounded-lg border-2 flex flex-col items-center justify-center p-1.5 transition-all duration-200 ${
                            player2
                              ? court.isPending
                          ...
                          onClick={player2 ? () => court.isPending ? removePlayerFromPendingCourt(player2.userId, court.id) : removePlayerFromCourt(player2.userId, court.id) : undefined}
                        >
                          {player2 ? (
                            <>
                              <Avatar className={`h-7 w-7 mb-0.5 border-2 ${teamRightColor.avatarBorder}`}>
                                <AvatarFallback className={`${teamRightColor.avatarBg} text-white text-[10px]`}>
                      ...
                      {/* 位置3 - 左下（左边队伍） */}
                      <div className="absolute bottom-2 left-2 right-1/2 top-1/2 flex items-center justify-center p-1.5">
                        <div
                          className={`w-full h-full rounded-lg border-2 flex flex-col items-center justify-center p-1.5 transition-all duration-200 ${
                            player3
                              ? court.isPending
                          ...
                          onClick={player3 ? () => court.isPending ? removePlayerFromPendingCourt(player3.userId, court.id) : removePlayerFromCourt(player3.userId, court.id) : undefined}
                        >
                          {player3 ? (
                            <>
                              <Avatar className={`h-7 w-7 mb-0.5 border-2 ${teamLeftColor.avatarBorder}`}>
                      ...
                      {/* 位置4 - 右下（右边队伍） */}
                      <div className="absolute bottom-2 right-2 left-1/2 top-1/2 flex items-center justify-center p-1.5">
                        <div
                          className={`w-full h-full rounded-lg border-2 flex flex-col items-center justify-center p-1.5 transition-all duration-200 ${
                            player4
                              ? court.isPending
                          ...
                          onClick={player4 ? () => court.isPending ? removePlayerFromPendingCourt(player4.userId, court.id) : removePlayerFromCourt(player4.userId, court.id) : undefined}
                        >
                          {player4 ? (
                            <>
                              <Avatar className={`h-7 w-7 mb-0.5 border-2 ${teamRightColor.avatarBorder}`}>
                    ...
                    {/* 待確認狀態 - 確認開始比賽 */}
                    {court.isPending && (
                      <div className="pt-3 border-t border-amber-500/50 bg-gradient-to-b from-amber-50/50 to-amber-100/50 dark:from-amber-950/20 dark:to-amber-900/30 -mx-6 px-6 pb-6 mt-4 rounded-b-lg">
                        <div className="space-y-3">
                          ...
                          <div className="flex gap-2">
                            <Button
                              variant="default"
                              size="lg"
                              className="flex-1 bg-green-600 hover:bg-green-700 text-base font-bold shadow-lg"
                    ...
                    {!court.isPending && court.isActive && court.players.length === 4 && (
                      <div className="pt-3 border-t border-red-500/30">
                        <Button
                          variant="destructive"
                          size="lg"
                          className="w-full bg-red-600 hover:bg-red-700 text-base font-bold shadow-lg"
                    ...
                    {/* 从等待队列分配玩家 */}
                    {!court.isPending && waitingQueue.length > 0 && court.players.length > 0 && court.players.length < 4 && (
                      <div className="pt-3 border-t">
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-xs text-muted-foreground font-medium">從等待隊列分配</p>
                          <Button
                            variant="default"
                            size="sm"
                        ...
                        <div className="space-y-1 max-h-32 overflow-y-auto">
                          {waitingQueue.slice(0, 5).map((member) => (
                            <Button
                              key={member.userId}
                              variant="outline"
                              size="sm"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {waitingQueue.map((member, index) => (
                ...
                >
                  ...
                  {index === 0 && (
                    <Badge className="bg-green-500/20 text-green-300 animate-pulse">
                      下一個
                    </Badge>
                  )}
                  {index > 0 && (
